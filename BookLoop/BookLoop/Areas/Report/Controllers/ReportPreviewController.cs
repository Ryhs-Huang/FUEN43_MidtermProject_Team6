    ...
    public class ReportPreviewController : Controller
    {
        ...
        public async Task<IActionResult> PreviewDraft([FromBody] PreviewReq req)
        {
            try
            {
                ...
                // ★ 書商篩選變數：用於使用者自訂的篩選條件 (與 Data Scope 分開處理)
                var filteredSupplierIds = new List<int>();
                bool supplierFilterSpecified = false;
                ...
                foreach (var f in req?.Filters ?? new())
                {
                    ...
                    // ★ 11) 書商/供應商 ID (用於自訂篩選，不影響 Data Scope)
                    else if (name == "supplierid" || name == "publisherid")
                    {
                        if (op == "in")
                        {
                            supplierFilterSpecified = true;
                            if (hasVJ)
                            {
                                var v = JsonSerializer.Deserialize<InV>(f.ValueJson!)!;
                                if (v.values?.Any() == true) filteredSupplierIds = v.values!.ToList();
                            }
                            else if (hasDV)
                            {
                                var tmp = f.DefaultValue!.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
                                                         .Select(s => int.TryParse(s, out var x) ? x : (int?)null)
                                                         .Where(x => x.HasValue).Select(x => x!.Value).ToList();
                                if (tmp.Any()) filteredSupplierIds = tmp;
                            }
                        }
                    }
                    ...
                    if (baseKind == "sales")
                    {
                        // === sales：以「書(BookID)」為單位，在日期+種類下統計銷售本數 ===
                        {
                            var q = from od in _shop.OrderDetails.AsNoTracking()
                                    ...
                                    select new { o.OrderDate, b.CategoryID, b.BookID, Title = b.Title, Qty = od.Quantity, SupplierID = p.SupplierID };

                            // ★ 修正後的 資料範圍和自訂篩選的合併邏輯
                            if (!canAll && canPub)
                            {
                                // 1. Data Scope 限制（書商身分，優先套用）
                                var supplierId = GetSupplierIdOrThrow();
                                q = q.Where(x => x.SupplierID == supplierId);

                                // 2. 如果書商還做了篩選，則將其視為在 Data Scope 內的「二次縮小」
                                if (supplierFilterSpecified)
                                {
                                    q = q.Where(x => filteredSupplierIds.Contains(x.SupplierID));
                                }
                            }
                            else if (supplierFilterSpecified)
                            {
                                // 3. 如果是 Admin/Marketing (canAll)，且做了篩選，則套用篩選
                                q = q.Where(x => filteredSupplierIds.Contains(x.SupplierID));
                            }
                            // 4. Admin/Marketing 且無篩選 → 不加限制 (這是對的)
                    ...
                    else if (baseKind == "borrow")
                    {
                        // === borrow：以「上架(ListingID)」為單位，在日期+種類下統計借閱次數 ===
                        {
                            var q = from br in _shop.BorrowRecords.AsNoTracking()
                                    ...
                                    select new { br.BorrowDate, l.CategoryID, l.ListingID, Name = l.Title, SupplierID = p.SupplierID };
                ...
                if (baseKind == "sales")
                {
                    var q = from od in _shop.OrderDetails
                            ...
                            join p in _shop.Publishers on b.PublisherID equals p.PublisherID
                ...
                else if (baseKind == "borrow")
                {
                    var q = from br in _shop.BorrowRecords
                            ...
                            join p in _shop.Publishers on l.PublisherID equals p.PublisherID
                            where br.BorrowDate >= dateFrom && br.BorrowDate <= dateTo
                            select new { br.BorrowDate, l.CategoryID, SupplierID = p.SupplierID };
