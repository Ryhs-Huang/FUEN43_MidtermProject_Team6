...
namespace Report.Areas.Report.Controllers
{
	...
	public class ReportExportController : Controller
	{
		...
		public async Task<IActionResult> SendExcel([FromBody] ReportExportDto dto)
		{
			...
			if (bytes == null || bytes.Length == 0) return BadRequest("無可匯出資料。");
			if (bytes.Length > MAX_BYTES) return BadRequest("匯出檔案過大，請縮小範圍或改為 PDF。");
			...
			using (var sha = SHA256.Create())
				checksum = sha.ComputeHash(bytes);
		...
		public async Task<IActionResult> SendPdf([FromBody] ReportExportDto dto)
		{
			...
			// 圖表 base64（允許含 data:image/png;base64, 前綴）
			...
			using (var ms = new MemoryStream())
			{
				...
				var headers = ResolveHeaders(
					dto.Category, dto.BaseKind ?? "", dto.Granularity ?? "", dto.Title, dto.ValueMetric);
				...
				if (hasGranularity) chips.Add($"粒度：{labelHeaderText}");
			...
			if (pdfBytes == null || pdfBytes.Length == 0) return BadRequest("無可匯出資料。");
			if (pdfBytes.Length > MAX_BYTES) return BadRequest("匯出檔案過大，請縮小範圍。");
			...
			sb.Append("<h3 style=\"margin:0 0 8px 0;\">").Append(HtmlEncode(dto.Title ?? "報表匯出")).AppendLine("</h3>");
			if (!string.IsNullOrWhiteSpace(dto.SubTitle))
				sb.Append("<div style=\"color:#555\">").Append(HtmlEncode(dto.SubTitle!)).AppendLine("</div>");
		...
		private async Task<int?> GetSupplierIdAsync(int userId)
		{
			...
			// 沒有則 DB fallback（注意是 _shop.SupplierUsers）
		...
		// 將 ReportExportDto 瘦身封存為快照（統一使用 Dto 版 ExportSnapshot）
		private static ExportSnapshot MakeSnapshot(ReportExportDto dto) => new ExportSnapshot
		{
